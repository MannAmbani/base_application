...
class InternalStorageContentProvider : ContentProvider() {
    companion object {
        val CONTENT_URI: Uri =
            Uri.parse("content://com.albinmathew.samples.photocrop/")
    ...
    override fun onCreate(): Boolean {
        return try {
            val mFile = File(
                context!!.filesDir,
                "" + System.currentTimeMillis() + ImageCropActivity.TEMP_PHOTO_FILE_NAME
            )
            if (!mFile.exists()) {
                mFile.createNewFile()
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        val f =
            File(context!!.filesDir, ImageCropActivity.TEMP_PHOTO_FILE_NAME)
        if (f.exists()) {
